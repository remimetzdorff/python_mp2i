...
ax1.plot([x.min(), x.max()], energy_mecanique(x0,v0)*np.ones(2), "--C3", alpha=.5)
...
def animate(i):
    ...
    Ec_profil.set_data([x[i], x[i]], [energy_ressort(x[i]), energy_mecanique(x[i], v[i])])
