...
# ANIMATION
###########
Ec_profil,    = ax1.plot([], [], "--C1", alpha=.5)
Ep_profil,    = ax1.plot([], [], "ok")
Em_profil,    = ax1.plot([], [], "oC0")
trajectoire,  = ax2.plot([], [], "ok", ms=10)

def animate(i):
    trajectoire.set_data([x[i]], [0])
    ax2.set_title("$x$ = {:.2f} m, $v$ = {:.2f} m/s".format(x[i], v[i]))
    Em_profil.set_data([x[i]], [energy_mecanique(x0, v0)])
    Ep_profil.set_data([x[i]], [energy_potentielle(x[i])])
    Ec_profil.set_data([x[i], x[i]], [energy_potentielle(x[i]), energy_mecanique(x0, v0)])
